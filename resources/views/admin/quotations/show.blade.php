            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-sm-6 text-right">
                        <h1 class="h3">Quotation #{{$quote->quote_no}}</h1>
                        <div>Issued at {{$quote->issue_date}}</div>
                        {{--<div class="text-danger m-b-15">Payment due April 21th, 2017</div>--}}
                        <br>
                        <address>
                            <strong>{{$quote->customer->full_name}}</strong><br>
                            {{$quote->customer->address}}<br>
                            {{$quote->customer->state}}, {{$quote->customer->zip}}<br>
                            <abbr title="Phone">M:</abbr> {{$quote->customer->mobile}}<br>
                            <strong>{{$quote->customer->email}}</strong><br>
                        </address>
                    </div>
                </div>
                <div class="table-responsive">
                    <table class="table table-striped table-nowrap">
                        ...
                        <tbody>
                        @foreach(json_decode($quote->product_details, true) as $product)
                        <tr>
                ...
                <div class="row">
                    <div class="col-sm-8">
                        <p>{{ !empty($profile->quotation_note) ? $profile->quotation_note : '' }}</p>
                    ...
                    <div class="col-sm-4">
                        <ul class="list-unstyled text-right">
                            <li>
                                <strong>Discount:</strong> {{$quote->total_discount}} /-</li>
                            <li>
                                <strong>GST:</strong> {{$quote->total_gst}} /-</li>
                            <li>
                                <strong>Grand Total:</strong> {{$quote->grand_total}} /-</li>
                        </ul>
                    </div>
                </div>
            </div>
