            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-sm-6 text-right">
                        <h1 class="h3">Invoice #{{$invoice->invoice_no}}</h1>
                        <h1 class="h3">Challan </h1>
                        @foreach($challans as $challan)
                            #{{$challan->challan_no}}
                        @endforeach
                        <div>Issued at {{$invoice->issue_date}}</div>
                        {{--<div class="text-danger m-b-15">Payment due April 21th, 2017</div>--}}
                        <br>
                        <address>
                            <strong>{{$invoice->customer->full_name}}</strong><br>
                            {{$invoice->customer->address}}<br>
                            {{$invoice->customer->state}}, {{$invoice->customer->zip}}<br>
                            <abbr title="Phone">M:</abbr> {{$invoice->customer->mobile}}<br>
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-nowrap">
                        ...
                        <tbody>
                        @foreach(json_decode($invoice->product_details, true) as $product)
                        <tr>
                ...
                <div class="row">
                    <div class="col-sm-8">
                        <p>{{ !empty($profile->invoice_note) ? $profile->invoice_note : '' }}</p>
                    ...
                    <div class="col-sm-4">
                        <ul class="list-unstyled text-right">
                            <li>
                                <strong>Discount:</strong> {{$invoice->total_discount}} /-</li>
                            <li>
                                <strong>GST:</strong> {{$invoice->total_gst}} /-</li>
                            <li>
                                <strong>Grand Total:</strong> {{$invoice->grand_total}} /-</li>
                        </ul>
                    </div>
                </div>
            </div>
